  ...
  async gameOver() {
    ...
    if (this.player.score > 0) {
      this.leaderboard.addEntry({
        ...
        username: this.player.username || "Anonymous",
...
function DungeonCrawler({ username }: { username: string }) {
  ...
  const usernameRef = useRef(username); // <-- keep latest username here

  // Keep usernameRef updated
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    game.player.username = gameUsername;
  ...
  // Update player username if prop changes mid-game
  useEffect(() => {
    if (gameRef.current && username) {
      if (
        username !== gameRef.current.player.username &&
...
function App() {
  ...
  const fetchUsername = async (walletAddress: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUsername(data.hasUsername ? data.user.username : `Player_${walletAddress.slice(0, 6)}`);
      } else {
        setUsername(`Player_${walletAddress.slice(0, 6)}`);
      }
    } catch {
  ...
  useEffect(() => {
    ...
    if (authenticated && user && ready && user.linkedAccounts.length > 0) {
      const crossAppAccount = user.linkedAccounts.find(
        (acc) =>
          acc.type === "cross_app" &&
          acc.providerApp.id === "cmd8euall0037le0my79qpz42"
      ) as CrossAppAccountWithMetadata;
  ...
  return (
    ...
    >
      ...
      {/* Game only shows if authenticated */}
