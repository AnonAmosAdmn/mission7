...
class Game {
  ...
  async generate_dungeon() {
      ...
      // Generate items
      this.items = [];
      const itemPromises: Promise<void>[] = []; // Array to track sprite loading promises
      
      for (let y = 0; y < this.map_height; y++) {
        for (let x = 0; x < this.map_width; x++) {
          if (this.tiles[y][x].type === 1) { // Only place items on floors
            if (Math.random() < 0.005) {
              ...
              // Start loading sprite immediately and track the promise
      ...
      // Wait for all sprites to load before proceeding
      await Promise.all(itemPromises);
      
      // Also ensure player and exit sprites are loaded
      await Promise.all([
        this.player.loadSprite(),
        this.exit.loadSprite()
      ]);
